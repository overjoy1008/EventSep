...
class ResUNet30(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        # ---------- NEW: optional temporal mask gating ----------
        if 'temporal_mask' in input_dict and input_dict['temporal_mask'] is not None:
            tm = input_dict['temporal_mask']
            waveform = output_dict['waveform']  # (B, C, T)

            # to tensor on same device / dtype
            if isinstance(tm, np.ndarray):
                tm = torch.tensor(tm, dtype=waveform.dtype, device=waveform.device)
            elif isinstance(tm, torch.Tensor):
                tm = tm.to(device=waveform.device, dtype=waveform.dtype)
            else:
                tm = torch.tensor(tm, dtype=waveform.dtype, device=waveform.device)

            # reshape: (T) -> (1,1,T), (B,T) -> (B,1,T)
            if tm.dim() == 1:
                tm = tm.unsqueeze(0).unsqueeze(0)       # (1,1,T)
            elif tm.dim() == 2:
                tm = tm.unsqueeze(1)                    # (B,1,T)
            elif tm.dim() == 3:
                pass  # assume already (B,1,T) or (B,C,T)
            else:
                raise ValueError("temporal_mask must have 1, 2, or 3 dims")

            B, C, T = waveform.shape

            # match batch dim
            if tm.shape[0] == 1 and B > 1:
                tm = tm.expand(B, -1, -1)

            # interpolate along time if needed
            if tm.shape[-1] != T:
                tm = F.interpolate(tm, size=T, mode='linear', align_corners=False)

            # broadcast along channel dim if needed
            if tm.shape[1] == 1 and C > 1:
                tm = tm.expand(B, C, T)

            output_dict['waveform'] = waveform * tm
        # -------------------------------------------------------
