...
def save_audio(
    ...
    """
    Minimal patch: Replace torchaudio.save with soundfile.write to avoid torchcodec requirement.
    """
    # clipping 방지
    ...
    if suffix == ".wav":
        # (C, T) → (T, C)
        wav_np = wav.detach().cpu().transpose(1, 0).numpy()
        ...
        # torchaudio.save 제거 → soundfile.write 사용
        sf.write(str(path), wav_np, samplerate, subtype=subtype)
        return

    # ========== FLAC (패치: torchaudio.save 제거) ==========
    if suffix == ".flac":
        wav_np = wav.detach().cpu().transpose(1, 0).numpy()
        sf.write(str(path), wav_np, samplerate, subtype="FLAC")
        return
