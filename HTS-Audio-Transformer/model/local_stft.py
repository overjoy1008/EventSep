...
from .local_pad_center import local_pad_center
...
class STFT(DFTBase):
    def __init__(self, n_fft=2048, hop_length=None, win_length=None,
        ...
        fft_window = local_pad_center(fft_window, n_fft)
    ...
    def forward(self, input):
        ...
        if self.center:
            ...
            if x.dim() == 4:
                # 4D input → must pad last dimension → need 6 integers
                pad = (0, 0, 0, 0, pad_amount, pad_amount)
            else:
                # 2D/3D input → original behavior
...
class ISTFT(DFTBase):
    ...
    def init_real_imag_conv(self):
        ...
        ifft_window = local_pad_center(ifft_window, self.n_fft)
    ...
    def init_overlap_add_window(self):
        ...
        ola_window = local_pad_center(ola_window, self.n_fft)
