...
# # from torchlibrosa.stft import Spectrogram, LogmelFilterBank
# from .safe_stft import SafeSpectrogram, SafeLogMel
# from torchlibrosa.augmentation import SpecAugmentation
...
#         self.interpolate_ratio = 32     # Downsampled ratio
#         # Spectrogram extractor
#         self.spectrogram_extractor = SafeSpectrogram(n_fft=config.window_size, hop_length=config.hop_size, 
#             win_length=config.window_size, window=window, center=center, pad_mode=pad_mode, 
#             freeze_parameters=True)
#         # Logmel feature extractor
#         self.logmel_extractor = SafeLogMel(sr=config.sample_rate, n_fft=config.window_size, 
#             n_mels=config.mel_bins, fmin=config.fmin, fmax=config.fmax, ref=ref, amin=amin, top_db=top_db, 
#             freeze_parameters=True)
#         # Spec augmenter
#         self.spec_augmenter = SpecAugmentation(time_drop_width=64, time_stripes_num=2, 
