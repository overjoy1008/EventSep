...
class LatentDiffusion(DDPM):
    ...
    def solve_euler(self, n_timesteps,batch_size,shape,cond=None, unconditional_guidance_scale=1.0, unconditional_conditioning=None, x_T=None,temperature=1.0,spks=None):
        ...
        for step in tqdm(range(1, len(t_span)), disable=True):  # EventSep: Evaluation에서 계속 나오는 것 방지
            ...
            for i, batch in enumerate(batchs):
                ...
                if self.extra_channels:
                    ...
                    if save_mixed:
                        ...
                        count = 0
                        for name in fnames:
                            save_path = os.path.join(mixed_save_path, os.path.basename(name))

                            # Tensor -> NumPy 변환
                            wav = mixed_waveform[count].cpu().numpy()
                            # 보통 (1, T) 형태이므로 (T,)로 펴 주기
                            if wav.ndim == 2 and wav.shape[0] == 1:
                                wav = wav[0]

                            sf.write(save_path, wav, self.sampling_rate)
                            count += 1
