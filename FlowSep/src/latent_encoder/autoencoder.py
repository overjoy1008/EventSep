...
class AutoencoderKL(pl.LightningModule):
    def __init__(
        ...
        # ============================================================
        # 💡 PATCH: Convert reload_from_ckpt to absolute FlowSep path
        # ============================================================
        if self.reload_from_ckpt is not None:
            ROOT = os.path.dirname(os.path.abspath(__file__))              # FlowSep/src/latent_encoder
            FLOWSEP_ROOT = os.path.abspath(os.path.join(ROOT, "..", "..")) # FlowSep/
            abs_ckpt = os.path.join(FLOWSEP_ROOT, self.reload_from_ckpt)

            if os.path.exists(abs_ckpt):
                self.reload_from_ckpt = abs_ckpt
            else:
                print(f"[WARN] Cannot find VAE ckpt at {abs_ckpt}")
        # ============================================================
