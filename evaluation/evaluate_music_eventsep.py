...
class MUSICEvaluator:
    ...
    def __init__(
        ...
        music_csv_path: str = "evaluation/metadata/music_eval.csv",
        music_audio_dir: str = "evaluation/data/music",
        ...
        debug_dir: str = "evaluation/debug/music_eventsep",
        ...
        with open(music_csv_path) as csv_file:
        ...
        self.audio_dir = music_audio_dir
        ...
        print(f"[MUSIC-EventSep] Using CLAPScore model: {self.clapscore_type}")
        print(f"[MUSIC-EventSep] Eval pairs: {len(self.eval_list)}")
        print(f"[MUSIC-EventSep] Debug samples: {len(self.debug_samples)} "
              f"will be saved in '{self.debug_dir}'")
        if self.use_demucs:
            print(f"[MUSIC-EventSep] Demucs semantic routing ENABLED "
    ...
    def _run_demucs_stem(self, mix_path: str, target: str) -> np.ndarray:
        ...
        tmp_in = f"/tmp/music_demucs_{tmp_id}.wav"
        tmp_out = f"/tmp/music_demucs_out_{tmp_id}"
    ...
    def __call__(self, pl_audiosep: pl.LightningModule, pl_flowsep: pl.LightningModule) -> Dict:
        print("Evaluation on MUSIC Test with [text label] queries (EventSep).")
        ...
        with torch.no_grad():
            for idx, caption, _, _ in tqdm(self.eval_list):
                ...
                text_query = caption  # instrument name
...
if __name__ == "__main__":
    print(
        "MUSICEvaluator (EventSep version).\n"
        "  evaluator = MUSICEvaluator(clapscore_type='audiosep')\n"
